...
module.exports = {
    ...
    .setName('setspoofylogs')
    .setDescription('Set the spoofylogs channel')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addChannelOption(option => option.setName('channel').setDescription('Set the channel to receive spoofy logs').setRequired(true)),
    /**
     * 
    ...
    async execute (interaction) {
        ...
        GuildSettings.findOne({ GuildID: interaction.guild.id }, (err, settings) => {
            if(err) {
                ...
                .setTitle("Error")
                .setDescription(`<a:deny:949724643089072209> **Unexpected Error!**\n <:blankspace:945334317603758090><a:arrow:945334977464262776> Could not set the spoofylog channel!`)
                .setColor("Red")
            ...
            if(!settings) {
                settings = new GuildSettings({
                    ...
                    Spoofylogs: interaction.options.getChannel('channel').id
                })
            } else {
                settings.Spoofylogs = interaction.options.getChannel('channel').id
            }
            ...
            settings.save(err => {
                if(err){
                    ...
                    .setTitle('<a:check:949722884576792596> Spoofylogs Channel')
                    .setColor("Green")
