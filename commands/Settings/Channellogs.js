...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('channel-logs')
        .setDescription('Channel-log system')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
        .addSubcommand(o => o.setName('enable').setDescription('Enable channel-logs').addChannelOption(o => o.setName('channel').setDescription('The channel to recieve channel-logs').setRequired(true)))
        .addSubcommand(o => o.setName('disable').setDescription('Disable channel-logs')),
    /**
     * 
                ...
                DB.findOne({GuildID: interaction.guild.id}, async (err, settings) => {
                    ...
                    if(!settings) {
                        settings = new DB({
                            ...
                            Channellogs: interaction.options.getChannel('channel').id
                        })
                    } else {
                        settings.ChannelLogSetting = true,
                        settings.Channellogs = interaction.options.getChannel('channel').id
                    }
                    ...
                    .setTitle(`${emote.check} Channel-log System`)
                    .setColor("Green")
                    .setDescription(`${emote.blank}${emote.arrow} **Setting:** Enabled\n${emote.blank}${emote.blank}${emote.channel} **Channel:** ${interaction.options.getChannel('channel')}`)
            ...
            if(sub === 'disable'){
                DB.findOne({GuildID: interaction.guild.id}, async (err, settings) => {
                    ...
                    if(!settings) {
                        settings = new DB({
                            GuildID: interaction.guild.id,
                            ChannelLogSetting: false,
                        })
                    } else {
                        settings.ChannelLogSetting = false
                    }
                    ...
                    const SettingsEmbed = new EmbedBuilder()
                    .setTitle(`${emote.check} Channel-log Channel`)
                    .setColor("Green")
