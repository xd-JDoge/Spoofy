...
module.exports = {
    ...
    .setName('leave-logs')
    .setDescription('Leave-logs system')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
    .addSubcommand(o => o.setName('enable').setDescription('Enable leave-logs').addChannelOption(o => o.setName('channel').setDescription('The channel to recieve leave-logs').setRequired(true)))
    .addSubcommand(o => o.setName('disable').setDescription('Disable leave-logs')),
    /**
     * 
                ...
                DB.findOne({GuildID: interaction.guild.id}, async (err, settings) => {
                    ...
                    if(!settings) {
                        settings = new DB({
                            ...
                            JoinLeave: true,
                            Leaveogs: interaction.options.getChannel('channel').id
                        })
                    } else {
                        settings.JoinLeave = true,
                        settings.Leavelogs = interaction.options.getChannel('channel').id
                    }
                    ...
                    .setTitle(`${emote.check} Leave-logs System`)
                    .setColor("Green")
