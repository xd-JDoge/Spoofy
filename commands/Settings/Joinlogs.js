...
module.exports = {
    ...
    .setName('join-logs')
    .setDescription('Join-logs system')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
    .addSubcommand(o => o.setName('enable').setDescription('Enable join-logs').addChannelOption(o => o.setName('channel').setDescription('The channel to recieve join-logs').setRequired(true)))
    .addSubcommand(o => o.setName('disable').setDescription('Disable join-logs')),
    /**
     * 
                ...
                DB.findOne({GuildID: interaction.guild.id}, async (err, settings) => {
                    ...
                    if(!settings) {
                        settings = new DB({
                            ...
                            JoinLeave: true,
                            Joinlogs: interaction.options.getChannel('channel').id
                        })
                    } else {
                        settings.JoinLeave = true,
                        settings.Joinlogs = interaction.options.getChannel('channel').id
                    }
                    ...
                    .setTitle(`${emote.check} Join-logs System`)
                    .setColor("Green")
